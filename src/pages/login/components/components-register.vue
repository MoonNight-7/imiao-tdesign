<template>
  ...
  >
    ...
    <t-form-item name="nickname">
      <t-input v-model="formData.nickname" size="large" placeholder="请输入您的昵称">
        <template #prefix-icon>
          <user-icon/>
        </template>
      </t-input>
...
const FORM_RULES = {
  ...
  gender: [{required: true, message: '性别必填', type: 'error'}],
    ...
    onSubmit({validateResult}: { validateResult: boolean }) {
      if (validateResult === true) {
        if (!this.formData.checked) {
          this.$message.error('请同意MoonNight服务协议和MoonNight 隐私声明');
          return;
        }
        custApi.userAddNew(this.formData).then((res) => {
